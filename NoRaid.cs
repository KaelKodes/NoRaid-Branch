...
/* CTS Kael Patches
k.1.3.31
Added protection from vehicles and other projectiles

k.1.3.0
Added Twig and Ladder support so owners can destroy other players entities on their base
*/
namespace Oxide.Plugins
{
    [Info("NoRaid", "Ryan, CTS Kael Branch", "1.3.31", ResourceId = 2530)]
    ...
    class NoRaid : RustPlugin
    {
        ...
        // Try very hard to attribute damage to a player (vehicle driver, gunner, rocket owner, etc.)
        private BasePlayer GetSourcePlayer(HitInfo info)
        {
            if (info == null) return null;

            // Best source: works for MLRS and many projectiles if ownerRef is set
            if (info.InitiatorPlayer != null)
                return info.InitiatorPlayer;

            var ent = info.Initiator as BaseEntity;
            return ResolvePlayerFromEntity(ent);
        }

        private BasePlayer ResolvePlayerFromEntity(BaseEntity ent)
        {
            // Walk up parents and check common cases: vehicles, mountables, ownerID
            int hops = 0;
            while (ent != null && hops++ < 8)
            {
                var p = ent.ToPlayer();
                if (p != null) return p;

                // Vehicle driver / any mounted occupant (attack heli, minis, subs, etc.)
                var vehicle = ent.GetComponentInParent<BaseVehicle>();
                if (vehicle != null)
                {
                    var driver = vehicle.GetDriver();
                    if (driver != null) return driver;

                    var seats = vehicle.GetComponentsInChildren<BaseMountable>();
                    if (seats != null)
                    {
                        foreach (var seat in seats)
                        {
                            var occ = seat.GetMounted();
                            if (occ != null) return occ;
                        }
                    }
                }

                // Simple mountable (gun seat / module)
                var mount = ent as BaseMountable;
                if (mount != null && mount.GetMounted() != null)
                    return mount.GetMounted();

                // Fallback: entity owner
                if (ent.OwnerID.IsSteamId())
                {
                    var owner = BasePlayer.FindByID(ent.OwnerID);
                    if (owner != null) return owner;
                }

                ent = ent.GetParentEntity() as BaseEntity;
            }
            return null;
        }
        ...
        private object OnEntityTakeDamage(BaseCombatEntity entity, HitInfo info)
        {
            ...
            {
                var player = GetSourcePlayer(info);
